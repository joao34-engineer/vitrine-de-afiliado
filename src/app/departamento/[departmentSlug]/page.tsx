...
export default async function DepartmentPage({ params, searchParams }: { params: Promise<{ departmentSlug: string }>; searchParams: Promise<{ pagina?: string; cursor?: string }> }) {
  ...
  return <CatalogShell mobileSearch="listing" mobileRail="listing"><CatalogListing eyebrow="Departamento" title={department.label} description={department.description} products={page.items} nextHref={nextHref} nextLabel={page.pageNumber === 10 ? "Proxima janela" : undefined} previousHref={previousHref} departmentSlug={department.slug} /></CatalogShell>;
}
