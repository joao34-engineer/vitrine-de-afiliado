...
export default async function LeafPage({ params, searchParams }: { params: Promise<{ leafSlug: string }>; searchParams: Promise<{ pagina?: string; cursor?: string }> }) {
  ...
  return <CatalogShell mobileSearch="listing"><CatalogListing eyebrow="Folha" title={leaf.label} description={leaf.description} products={page.items} nextHref={nextHref} nextLabel={page.pageNumber === 10 ? "Proxima janela" : undefined} previousHref={previousHref} departmentSlug={leaf.departmentSlug} leafSlug={leaf.slug} /></CatalogShell>;
}
